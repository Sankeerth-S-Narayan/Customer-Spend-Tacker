...
def create_charts_view(df: pd.DataFrame) -> pn.Column:
    ...
    if data['amount'].sum() == 0:
        ...
        pie_chart = figure(height=350, title="Spending Proportion by Category", toolbar_location=None,
               tools="hover", tooltips="@category: @amount{($0,0.00)}", x_range=(-0.5, 1.0),
               sizing_mode="stretch_width")
    ...
    charts_layout = pn.Column(
        pn.Row(bar_chart, pie_chart, sizing_mode="stretch_width"),
        line_chart,
        sizing_mode="stretch_width"
    )
